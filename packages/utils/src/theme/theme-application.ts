...
import { FLYERS_SOFT_THEME, getBrandMapping, getNeutralMapping, invertPalette } from "./theme-inversion";
...
export function applyCustomTheme(brandColor: string, neutralColor: string, mode: "light" | "dark"): void {
  ...
  const flyersNeutralColor = mode === "dark" ? FLYERS_SOFT_THEME.darkBackground : FLYERS_SOFT_THEME.lightBackground;

  // Generate palettes directly in OKLCH color space
  const { brandPalette, neutralPalette } = generateThemePalettes(
    FLYERS_SOFT_THEME.primaryPurple,
    flyersNeutralColor,
    mode
  );
  const neutralOKLCH = hexToOKLCH(flyersNeutralColor);
  ...
  const { textColor, iconColor } = getOnColorTextColors(FLYERS_SOFT_THEME.primaryPurple, "wcag");
