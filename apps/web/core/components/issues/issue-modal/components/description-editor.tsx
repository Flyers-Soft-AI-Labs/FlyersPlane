...
import { Bold, Code2, Italic, Link2, List, ListOrdered, Quote, Sparkle } from "lucide-react";
// plane imports
...
export const IssueDescriptionEditor = observer(function IssueDescriptionEditor(props: TIssueDescriptionEditorProps) {
  ...
  const runEditorCommand = (itemKey: "bold" | "italic" | "bulleted-list" | "numbered-list" | "quote" | "code") => {
    editorRef.current?.executeMenuItemCommand({ itemKey });
  };

  const handleLinkCommand = () => {
    const url = window.prompt("Enter or paste a link");
    if (!url) return;

    editorRef.current?.executeMenuItemCommand({ itemKey: "link", url });
  };

  const toolbarItems = [
    { label: "Bold", icon: Bold, onClick: () => runEditorCommand("bold") },
    { label: "Italic", icon: Italic, onClick: () => runEditorCommand("italic") },
    { label: "Link", icon: Link2, onClick: handleLinkCommand },
    { label: "Bulleted list", icon: List, onClick: () => runEditorCommand("bulleted-list") },
    { label: "Numbered list", icon: ListOrdered, onClick: () => runEditorCommand("numbered-list") },
    { label: "Quote", icon: Quote, onClick: () => runEditorCommand("quote") },
    { label: "Code", icon: Code2, onClick: () => runEditorCommand("code") },
  ];
  ...
  return (
    <div className="flyers-soft-description-editor relative rounded-lg border-[0.5px] border-subtle-1 bg-layer-2">
      {descriptionHtmlData === undefined || !projectId ? (
        ...
        <>
          <div className="flyers-soft-description-toolbar flex items-center gap-1 border-b border-subtle-1 px-3 py-2">
            {toolbarItems.map((item) => {
              ...
              return (
                <button
                  key={item.label}
                  type="button"
                  className="flyers-soft-description-toolbar-button"
                  title={item.label}
            ...
            <span className="flyers-soft-description-toolbar-spacer" />
            {issueName && issueName.trim() !== "" && config?.has_llm_configured && (
              <button
                type="button"
                className={`flyers-soft-description-toolbar-button ${iAmFeelingLucky ? "cursor-wait" : ""}`}
          ...
          <Controller
            ...
            render={({ field: { value, onChange } }) => (
              <RichTextEditor
                ...
                containerClassName="flyers-soft-description-editor-content pt-3 min-h-[150px]"
